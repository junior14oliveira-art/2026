/*
    ...
    Functions to access the transport driver interface (TDI).
...
#ifdef _PREFAST_
IO_COMPLETION_ROUTINE tdi_mdl_completion;
#endif // _PREFAST_

static NTSTATUS tdi_mdl_completion(PDEVICE_OBJECT deviceObject, PIRP irp, PVOID context)
{
...
NTSTATUS tdi_send_dgram_mdl(PFILE_OBJECT addressFileObject, ULONG addr, USHORT port, PMDL mdl)
{
    ...
    TdiBuildSendDatagram(irp, devObj, addressFileObject, tdi_mdl_completion, NULL, len ? mdl : 0, len, remoteInfo);
...
NTSTATUS tdi_recv_dgram_mdl(PFILE_OBJECT addressFileObject, PULONG addr, PUSHORT port, PMDL mdl, ULONG flags)
{
    PDEVICE_OBJECT              devObj;
    KEVENT                      event;
    PTDI_CONNECTION_INFORMATION remoteInfo;
    ...
    TdiBuildReceiveDatagram(irp, devObj, addressFileObject, tdi_mdl_completion, NULL, len ? mdl : 0, len, remoteInfo, returnInfo, flags);
...
NTSTATUS tdi_send_stream_mdl(PFILE_OBJECT connectionFileObject, PMDL mdl, ULONG flags)
{
    ...
    TdiBuildSend(irp, devObj, connectionFileObject, tdi_mdl_completion, NULL, len ? mdl : 0, flags, len);
...
NTSTATUS tdi_recv_stream_mdl(PFILE_OBJECT connectionFileObject, PMDL mdl, ULONG flags)
{
    PDEVICE_OBJECT  devObj;
    KEVENT          event;
    PIRP            irp;
    ...
    TdiBuildReceive(irp, devObj, connectionFileObject, tdi_mdl_completion, NULL, len ? mdl : 0, flags, len);
...
#ifdef _PREFAST_
IO_COMPLETION_ROUTINE tdi_send_async_completion;
#endif // _PREFAST_

static NTSTATUS tdi_send_async_completion(PDEVICE_OBJECT deviceObject, PIRP irp, PVOID context)
{
...
NTSTATUS tdi_send_stream_async(PFILE_OBJECT connectionFileObject, PMDL mdl, ULONG flags, void (*completionRoutine)(int status, void *context), void *context)
{
    PDEVICE_OBJECT          devObj;
    PIRP                    irp;
    ...
    TdiBuildSend(irp, devObj, connectionFileObject, tdi_send_async_completion, ctx, len ? mdl : 0, flags, len);
