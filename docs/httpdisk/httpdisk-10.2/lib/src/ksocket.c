...
int __cdecl recv_mdl(INT_PTR socket, PMDL mdl, int flags)
{
    PSOCKET s = (PSOCKET) -socket;

    if (s->type == SOCK_DGRAM)
    {
        return recvfrom_mdl(socket, mdl, flags, 0, 0);
    }
    else if (s->type == SOCK_STREAM)
    {
        if (!s->isConnected)
...
int __cdecl recvfrom_mdl(INT_PTR socket, PMDL mdl, int flags, struct sockaddr *addr, int *addrlen)
{
    PSOCKET s = (PSOCKET) -socket;
    struct sockaddr_in* returnAddr = (struct sockaddr_in*) addr;
    ...
    if (s->type == SOCK_STREAM)
    {
        return recv_mdl(socket, mdl, flags);
    }
    else if (s->type == SOCK_DGRAM)
    {
        u_long* sin_addr = 0;
...
int __cdecl send_async(INT_PTR socket, PMDL mdl, int flags, void (*completion_routine)(int status, void *context), void *context)
{
    PSOCKET s = (PSOCKET) -socket;
